...
if __name__ == '__main__':
    ...
    paths = {'path': r'/home/centos/merge/merge_netcdf',
             'export': r'/home/centos/merge/figuras',
             'logo': r'/home/centos/merge'
            }

    nomes = {'nome_logo': r'logo.png'}

    #  Periodo em que se realizara o somatorio
    periodo = {'de': '2018-05-17',
              'ate': '2018-05-17'
               }
    ...
    #print(glob.glob('/home/centos/merge/merge_netcdf/*nc'))
    lista = glob.glob('/home/centos/merge/merge_netcdf/*nc')
    ...
    shp = m.readshapefile(r'/home/centos/merge/shapefile/BRA_adm1', 'states',
                          drawbounds=True
                          )
    ...
    #newax = fig.add_axes([0.53, 0.16, 0.2, 0.2], anchor='SE', zorder=+1)
    #newax.imshow(im, alpha=0.8)
    #newax.axis('off')
    print(os.path.join(paths['export'], 'Observado_{:%Y%m%d}_{:%Y%m%d}.png'.format(periodo['de'], periodo['ate'])
                             ))
    plt.savefig(os.path.join(paths['export'], 'Observado_{:%Y%m%d}_{:%Y%m%d}.png'.format(periodo['de'], periodo['ate'])
                             ),
                bbox_inches='tight'
                )
    plt.show()
